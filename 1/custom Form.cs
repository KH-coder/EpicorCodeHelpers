...
public class Script
{
	...
	PickedConfirm obj=new PickedConfirm();
	...
	{
		PubFun.AddStatusPanel(oTrans.EpiBaseForm,"Ver:1.01.20230625", "ToDo");
		obj.FormLoad("Picked Confirm");
	}
}
    public class PickedConfirm
    {
        ...
        DataTable FilterDT;
        DataTable DataListDT;

        EpiDataView edvFilter;
        EpiDataView edvDataList;
        //
        EpiGroupBox epiGBFilter;
        EpiButton epiBtQuery;
        EpiUltraGrid epiUGDataList;
        ...
        public void Initialize(Ice.Lib.Customization.CustomScriptManager csm1)
        {
            ...
            LanguageID = ((Ice.Core.Session)(oTrans.Session)).LanguageID;
            UserID = ((Ice.Core.Session)(oTrans.Session)).UserID;
            udForm = ((EpiBaseForm)(csm.GetGlobalInstance(UDName + "Form")));
            InitEpiDataView();
            //Get Controls
            epiGBFilter = PubFun.GetControlByName<EpiGroupBox>(csm, "epiGBFilter");
            epiBtQuery = PubFun.GetControlByName<EpiButton>(csm, "epiBtQuery");
            epiUGDataList = PubFun.GetControlByName<EpiUltraGrid>(csm, "epiUGDataList");
            //
            epiBtQuery.Click += EpiBtQuery_Click;
            DataListDT.ColumnChanging += DataListDT_ColumnChanging; 
            //
            PubFun.SetEDVReadWrite(edvDataList, new List<string> { "Calculated_Picked" });
        ...
        {
            epiGBFilter.Dock = DockStyle.Fill;
            epiUGDataList.Dock = DockStyle.Fill;
            PubFun.InitEpiUltraGridB(epiUGDataList, GetDataListColumns(), true, LanguageID);
            if (DataListDT.Columns.Contains("Calculated_Picked"))
            {
                epiUGDataList.DisplayLayout.Bands[0].Columns["Calculated_Picked"].Header.CheckBoxVisibility = HeaderCheckBoxVisibility.Always;
                epiUGDataList.DisplayLayout.Bands[0].Columns["Calculated_Picked"].Header.Caption = "Picked";
                epiUGDataList.DisplayLayout.Bands[0].Columns["Calculated_Picked"].Header.CheckBoxAlignment = HeaderCheckBoxAlignment.Center;
            }
        }
        private void InitEpiDataView()
        {
            ...
            FilterDT.Columns.AddRange(new DataColumn[] {
                 PubFun.NewColumn("DateF","","",80,typeof(DateTime))
                ,PubFun.NewColumn("DateT","","",80,typeof(DateTime))
                ,PubFun.NewColumn("PackIDF","","",80,typeof(int))
                ,PubFun.NewColumn("PackIDT","","",80,typeof(int))
                ,PubFun.NewColumn("TruckIDF")
                ,PubFun.NewColumn("TruckIDT")
                ,PubFun.NewColumn("Picked","","",80,typeof(bool))
                ,PubFun.NewColumn("SysRowID")
            });
            DataRow dr = FilterDT.NewRow();
            FilterDT.Rows.Add(dr);
            edvFilter = new EpiDataView();
            edvFilter.dataView = new DataView(FilterDT);
            if ((oTrans.EpiDataViews.ContainsKey("Filter") == false))
            {
                oTrans.Add("Filter", edvFilter);
            }

            //DataList
            DataListDT = new DataTable();
            DataListDT.Columns.AddRange(GetDataListColumns());
            edvDataList = new EpiDataView();
            edvDataList.dataView = new DataView(DataListDT);
            if ((oTrans.EpiDataViews.ContainsKey("DataList") == false))
            {
                oTrans.Add("DataList", edvDataList);
            }
        }

        private DataColumn[] GetDataListColumns()
        {
            return new DataColumn[] {
                 PubFun.NewColumn("Calculated_PackID","Pack ID","",60,typeof(int))
                 ,PubFun.NewColumn("Calculated_Date","Date","",100,typeof(DateTime))
                 ,PubFun.NewColumn("Calculated_TruckID","Truck ID","",60)
                 ,PubFun.NewColumn("Calculated_CustName","Customer Name","",150)
                 ,PubFun.NewColumn("Calculated_Trun","Trun","",60,typeof(int))
                 ,PubFun.NewColumn("Calculated_TimeF","From","",70,typeof(DateTime),true,false,0,"HH:mm")
                 ,PubFun.NewColumn("Calculated_TimeT","To","",70,typeof(DateTime),true,false,0,"HH:mm")
                 ,PubFun.NewColumn("Calculated_Picked","Picked","",70,typeof(bool))
            };
        }
        private void EpiBtQuery_Click(object sender, EventArgs e)
        {
            try
            {
                PubFun.BeginLoadData(DataListDT);
                DataListDT.Clear();
                oTrans.StatusPanel.Text = "Query Data...";
                Application.DoEvents();
                List<PubBAQ.Parameter> list = new List<PubBAQ.Parameter>();
                if (Convert.IsDBNull(edvFilter.CurrentDataRow["DateF"]) == false)
                {
                    list.Add(new PubBAQ.Parameter("DateF", Convert.ToDateTime(edvFilter.CurrentDataRow["DateF"]).ToString("yyyy-MM-dd"), PubBAQ.DataType.Date));
                }
                if (Convert.IsDBNull(edvFilter.CurrentDataRow["DateT"]) == false)
                {
                    list.Add(new PubBAQ.Parameter("DateT", Convert.ToDateTime(edvFilter.CurrentDataRow["DateT"]).ToString("yyyy-MM-dd"), PubBAQ.DataType.Date));
                }
                if (Convert.ToInt32(edvFilter.CurrentDataRow["PackIDF"]) != 0)
                {
                    list.Add(new PubBAQ.Parameter("PackIDF", Convert.ToInt32(edvFilter.CurrentDataRow["PackIDF"]).ToString(), PubBAQ.DataType.Int));
                }
                if (Convert.ToInt32(edvFilter.CurrentDataRow["PackIDT"]) != 0)
                {
                    list.Add(new PubBAQ.Parameter("PackIDT", Convert.ToInt32(edvFilter.CurrentDataRow["PackIDT"]).ToString(), PubBAQ.DataType.Int));
                }
                if (Convert.ToString(edvFilter.CurrentDataRow["TruckIDF"]) != "")
                {
                    list.Add(new PubBAQ.Parameter("TruckIDF", Convert.ToString(edvFilter.CurrentDataRow["TruckIDF"]), PubBAQ.DataType.Nvarchar));
                }
                if (Convert.ToString(edvFilter.CurrentDataRow["TruckIDT"]) != "")
                {
                    list.Add(new PubBAQ.Parameter("TruckIDT", Convert.ToString(edvFilter.CurrentDataRow["TruckIDT"]), PubBAQ.DataType.Nvarchar));
                }
                list.Add(new PubBAQ.Parameter("Picked", Convert.ToBoolean(edvFilter.CurrentDataRow["Picked"]).ToString(), PubBAQ.DataType.Bit));
                DataTable dt = PubBAQ.GetBAQData(oTrans, "BK_BAQ_PickedConfirm", list.ToArray());
                PubFun.CopyTable(dt, DataListDT);
                if (edvDataList.dataView.Count > 0) edvDataList.Row = 0;
                edvDataList.Notify(new EpiNotifyArgs(oTrans, edvDataList.Row, EpiTransaction.NotifyType.Initialize));
            }
            finally
            {
                oTrans.StatusPanel.Text = "Ready";
                PubFun.EndLoadData(DataListDT);
            }
        }

        private void DataListDT_ColumnChanging(object sender, DataColumnChangeEventArgs e)
        {
            if (PubFun.IsLoadData(DataListDT) == false)
            {
                switch (e.Column.ColumnName)
                {
                    case "Calculated_Picked":
                        bool picked = Convert.ToBoolean(e.ProposedValue);
                        Erp.Adapters.CustShipAdapter adp = new Erp.Adapters.CustShipAdapter(oTrans);
                        adp.BOConnect(); 
                        PubFun.BeginLoadData(DataListDT);
                        try
                        {
                            if (adp.GetByID(Convert.ToInt32(e.Row["Calculated_PackID"])))
                            {
                                if (Convert.ToBoolean(adp.CustShipData.ShipHead.Rows[0]["ReadyToInvoice"]))
                                {
                                    MessageBox.Show("Delivered has been checked, and Picked cannot be modified.","Error");
                                    e.ProposedValue = e.Row["Calculated_Picked"];
                                }
                                else if (Convert.ToBoolean(adp.CustShipData.ShipHead.Rows[0]["Printed_c"])==false && picked)
                                {
                                    MessageBox.Show("Printed has not been checked, and Picked cannot be checked.", "Error");
                                    e.ProposedValue = e.Row["Calculated_Picked"];
                                }
                                else
                                {
                                    adp.CustShipData.ShipHead.Rows[0].BeginEdit();
                                    adp.CustShipData.ShipHead.Rows[0]["Picked_c"] = picked;
                                    adp.CustShipData.ShipHead.Rows[0].EndEdit();
                                    adp.Update();
                                }
                            }
                        }
